  ...
  async getLiquidityForSpread(params: GetLiquidityParams): Promise<BigNumber> {
    ...
    Object.keys(params.orderBook).map(outcomeId => {
      Object.keys(params.orderBook[outcomeId]).forEach(orderType => {
        ...
        params.orderBook[outcomeId][orderType] = _.filter(
          params.orderBook[outcomeId][orderType],
          order => {
            const gasCost = new BigNumber(order.amount)
              .multipliedBy(params.numTicks)
              .div(MAX_TRADE_GAS_PERCENTAGE_DIVISOR);
